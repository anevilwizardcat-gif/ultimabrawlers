...
## P110 - VS/"NEXT MATCH" screen: old starfield replaced with char-select crisp twinkle (system.def)
- Symptom (Raven): the VS screen (Image: NEXT MATCH / VS / "Next Stage") still had the OLD ugly star look; it never got the P105/P107 crisp-pixel twinkle that char-select did.
- ROOT: the VS screen is its OWN [VersusBGdef] using DIFFERENT sprite groups than char-select. Old layers: back [VersusBG Background Clouds Top] = sprite 102,2 (type=normal, vel -0.05, trans=add1); front [VersusBG Background Clouds Bottom] = 102,3 (type=normal, vel -0.7). Char-select (group 101) was upgraded in P107 but group 102 was left behind.
- FIX (pure system.def, ZERO sff change): back layer -> type=anim, actionno=250 (REUSE char-select's existing 4-frame varied-twinkle anim that draws 101,2/30/31/32), vel matched to -0.1, dropped trans=add1 so the crisp stars render opaque like char-select. Front 102,3 layer SCRAPPED (commented out) exactly as char-select scrapped its 101,3 front layer in P107.
- WHY reuse actionno 250 instead of new VS frames: [Begin Action N] blocks are global; one anim can back multiple [*BGdef] elements. Reusing 250 guarantees the VS starfield is byte-identical in look to char-select with no new sprites and no SFF surgery. Verified 101,2/30/31/32 all present in sff.
- FIREWALL: VS screen = [VersusBGdef] / groups 102+204; char-select = [SelectBGdef] / group 101; title = [TitleBGdef] / group 100. Three separate BGdefs. This edit touched ONLY VersusBGdef. sky (204,0) and VS/NEXT MATCH logos (anim 200/202) left alone (Raven only asked for the star texture).

## P111 - char-select TEAM-MENU arrow + player icons restyled to screenpack gold/cyan (system.sff)
- Symptom (Raven): team-mode selector = default BLUE triangle arrow; the two person/value icons next to Tag are BLUE on P1 side and pinkish-RED on P2 side. Wanted "golden arcadey/anti-aliased pixel" in our style.
- These are BAKED motif sprites, not recolorable via a key. Motif keys (system.def [Select Info]) already point at them - NO def change needed:
  - arrow cursor: p1.teammenu.item.cursor.anim=180 / p2...=190  -> [Begin Action 180]/[190] draw sprite groups 180 (frames 0/1/2) and 190 (0/1/2). 3-frame pulse (big->med->sliver).
  - value icons: p1.teammenu.value.icon.spr=181,0 (filled) / .empty.icon.spr=182,0 ; p2 = 191,0 / 192,0. 30x48, axis(14,45).
- RESTYLE METHOD: luminance-remap recolor (preserves EXACT silhouette, bevel, gloss, glow, AA edges, dims, axes - only hue shifts). P1 -> screenpack gold (ramp shadow(52,30,2)/mid(238,182,32)/bright(255,242,158)); P2 -> screenpack cyan (2,36,56 / 48,182,228 / 196,246,255). "empty" icons use a dim ramp. Matches the settled P1=gold(255,210,40)/P2=cyan(90,225,255) scheme used everywhere else in the motif.
- 10 sprites replaced: 180,0/1/2 + 190,0/1/2 (arrows), 181,0/182,0 + 191,0/192,0 (icons).
- SFFv2 OVERWRITE (no count change): for each, append [4-byte uncompressed-size][PNG] at true EOF, patch node dataOffset(+16)/dataLen(+20) (W/H/fmt12/cd32/flags bit0=0 unchanged). Node table NOT relocated, count stays 300. Old sprite data orphaned in ldata (harmless). VERIFIED: all 277 PNG sprites round-trip-decode, 0 failures; all 10 targets correct dims.
- DECISION/ASSUMPTION: made P2 arrow+icons CYAN (not gold) to match the established P1-gold/P2-cyan convention across the screenpack. If Raven wants BOTH sides gold, only the P2 ramp needs swapping (trivial re-bake).
- FIREWALL: ui-motif (motif + sprites). No CNS/explods/states. The luminance-remap recolor trick is a sprite-art method; unrelated to any meter/gauge work.

## P112 - VS screen: OLD STARS WERE BAKED INTO SKY 204,0 (the real root; P110 was necessary but not sufficient)
- P110 swapped the VS scroll LAYER to the twinkle anim but the screen STILL looked old. Root: the two big magenta 5-point star OUTLINES + scattered purple crosses/diamonds are BAKED INTO THE STATIC VS SKY SPRITE 204,0 (1280x960) - exactly the same situation P105 found+fixed on the CHAR-SELECT sky 101,0. Swapping a scroll layer can never remove baked-in sky art.
- LESSON (firewall note): for these screenpack backdrops, "stars" can live in TWO places - (a) the scroll-layer anim, (b) BAKED into the static *,0 sky sprite. Always extract and EYEBALL the sky sprite before concluding a star edit is done. Title=100,0 / select=101,0 / versus=204,0 each have their own baked sky.
- FIX: stripped 204,0 the P105 way - detect bright lavender-magenta star/symbol pixels (R>105 & B>120 & G<(R+B)/2-20), exclude gold deco (R>150&G>95&B<130&R>B+30), dilate mask 1px for AA fringe, refill each row with that row's plum-gradient median. Gold diagonal lines + halftone + vertical gradient fully preserved. Result matches the clean select sky 101,0.
- SFFv2 OVERWRITE: append [4B size][PNG] at EOF, patch 204,0 node off(+16)/len(+20), dims/fmt12/cd32 unchanged. Count stays 300. All 277 PNG sprites round-trip-decode, 0 fail; 204,0 verified 1280x960.
- COMBINED with P110 (scroll layer -> anim 250 twinkle + front 102,3 scrapped), the VS screen now fully matches char-select: clean gold sky + crisp moving twinkle.

## P113 - VS/"loading" screen: the REAL fix = rebake the scroll-layer SPRITES 102,2/102,3 (NOT a def layer swap, NOT 204,0)
- CORRECTION of P110/P112. Raven: "we dont have a sky in our loading screen... its just a purple bg + yellow lines + stars you made for it." Correct. The visible ugly stars = the SCROLL-LAYER SPRITES themselves: 102,3 (2467x960) is a dense GRID of colorful symbols (snowflakes/diamonds/sparkles) + 102,2 (2366x505) faint diamonds - both BAKED by P036. The look is IN the sprite pixels.
- WHY P110/P112 failed: P110 only changed the def LAYER (102,2 -> type=anim actionno 250). That depends on (a) Raven running my system.def AND (b) the cross-group anim resolving. Repo def != live def (repo lags), so the live screen kept the old SPRITE pixels. P112 stripped 204,0 = the engine-default "sky" layer, which Raven says is disabled/not what renders - wrong target.
- FIX (sprite-carried = robust to def state): rebaked 102,2 + 102,3 PNGs as crisp pixel starfields matching char-select 101,2 EXACTLY (sampled its palette: white 240,240,240 dominant + lavender 144,168,240 / purple 192,168,240 / cyan 144,216,240 / gold 240,216,120 / pink 240,168,192; hard-edged dots/plus/spark; ~0.9% density back, ~0.4% front; seamless HORIZONTAL wrap so the scroll tiles). 102,2 = main field (slow), 102,3 = sparse brighter (fast parallax).
- def [VersusBGdef]: reverted P110's anim-250 layer to PLAIN type=normal scroll on 102,2 + 102,3 (dropped the fragile cross-group anim dependency). The crisp look now lives in the sprite, so it shows whether Raven uses my def or his live def.
- SFFv2 OVERWRITE 102,2 + 102,3 (append [4B size][PNG] at EOF, patch node off/len; same dims; count stays 300). All 277 PNG sprites decode, 0 fail. 204,0 left as the P112-cleaned version (harmless; not the renderer per Raven).
- LESSON (firewall): a "star layer" can be (a) baked into the *,0 sky sprite, (b) baked into the SCROLL sprite (102,2/102,3 here - THIS case), or (c) a def-driven anim. When a def-layer edit "doesn't take", the look is almost certainly baked in the SPRITE and repo!=live hid it. Fix the SPRITE, don't chase the def.

## P114 - star layers: fix "runs out after AFK" (ENGINE root cause) + VS match char-select (DEF-ONLY, on Raven's LIVE file)
- Worked from Raven's UPLOADED LIVE system.def/.sff (ground truth) after the folder mixup (files go to data/ikemen1/, NOT data/) was cleared up. Live file = my P113 state.
- Raven's 4 issues: VS had (1) two star layers, (2) no sparkle, (3) stars too tiny; and (4) title+select+VS star layers "run out" after long AFK.
- *** ENGINE ROOT CAUSE of the run-out (read Ikemen-GO src) ***: bgAction.action() [src/stage.go ~L92] does `bga.pos[i] += bga.vel[i]` every tick with NO wrap -> a constant `velocity` makes the scroll offset grow UNBOUNDED. The tile fill (rmTileHSub, src/render.go) computes left/right tile bounds from that offset; at large offsets the bounds break down (left>=right) and it draws ZERO tiles -> stars vanish. Faster layers (-0.7) die sooner than slow (-0.1). tile=1,0 is NOT the problem (tile.x==1 = "refill whole screen every frame"; can't run out of tiles). Frames are uniform width (no drift). It is specifically the UNBOUNDED velocity offset.
- FIX (def-only): on all 3 star layers (TitleBG/SelectBG/VersusBG Clouds Top) set `velocity = 0,0` and add `sin.x = 300, 9000, 0`. sin offset = radius*sin(2pi*t/looptime) is BOUNDED [-300,+300] -> perpetual gentle drift that can NEVER run out. Keeps the slide + the twinkle anim. (sin.x syntax = amplitude, looptime, starttime, per src/stage.go L312-339.)
- VS-specific (issues 1-3): [VersusBG Background Clouds Top] -> type=anim, actionno=250 (REUSE char-select twinkle frames 101,2/30/31/32) = identical sparkle + identical (bigger) star size as char-select, no separate VS sprites. Removed [VersusBG Background Clouds Bottom] (the 2nd layer). P113's tiny 102,2/102,3 sprites now unreferenced (left in sff, harmless).
- system.sff UNCHANGED this round. Only system.def shipped.
- NOTE: Victory/Option/Replay/Hiscore BGdefs likely have the same velocity run-out if they use star layers - left untouched (not requested); same one-line sin.x fix applies if wanted.
- ENGINE FACT for future me: motif backgrounds with autonomous constant `velocity` ALWAYS eventually run out (unbounded bga.pos). For perpetual motion use `sin.x`/`sin.y` (bounded) or velocity=0. Stages don't hit this because they scroll via camera `delta`, not `velocity`.

## P115 - YOU WIN shimmer restored + lifebar text -> combo-gold + P2 menu color cyan->purple
SUBSYSTEMS (kept isolated): (1) lifebar-layout (fight.def anims+fonts), (2) ui-motif (system.sff P2 sprites). No CNS/explods touched.
(1a) YOU WIN! FLASH RESTORED [fight.def]: anims 580 (p1) / 581 (p2) held a SINGLE static frame 530,10 for 150 ticks (slide-in/out via Interpolate Offset was fine = "physical anim works, flash gone"). The shimmer frames 530,10..530,15 (6 shine frames, 630x145) EXIST in fight.sff (v2). Rebuilt the on-screen hold as 24 cycling frames (10->15 x4 @6t = ~150t) at the same offset, slide-in/out Interpolate Offset preserved. DEF-ONLY (frames already in sff). LOSE anims 582/583 (530,20) left slide-only per design. NOTE: can't use LoopStart (loops forever, never reaches slide-out) -> unrolled the cycle.
(1b) LIFEBAR TEXT -> COMBO AESTHETIC [fight.def]: all silver font8 (Pixel.def) text swapped to font6 (Arcade.def Jersey10) bank 7 = GOLD, matching the combo counter. Sections: [Name]+Simul/Tag variants (p1-p8), [Score], [WinCount], and [AILevel] (= the "CPU-%s" lvl text above the bar). Shown benched p3/p4 names kept their palfx.mul grey -> greyed-GOLD (tagged-out look). Tag_2P benched p3/p4 left HIDDEN (P041 design) - styled template ready if Raven wants them shown. Arcade.def banks: 0=mono 1=red 2=grn 3=blu 4=brn 5=cyan 6=purple 7=yellow/gold. Left [Timer]/[Match]/[Mode]demo on font8 (mode-specific overlays, not normal-play HUD).
(2) P2 MENU COLOR cyan->LIGHT PURPLE [live system.sff]: P111 made P2 cyan; Raven wants light purple that contrasts the plum bg. Recolored sprites 190,0/1/2 (arrow) + 191,0 (icon full) + 192,0 (icon empty) via luminance-remap to lavender ramp (full: 34,14,60 / 150,96,228 / 224,204,255; dim: 24,16,40 / 86,64,122 / 150,132,184). SFFv2 overwrite (append+patch, count stays). All 277 png decode. system.def UNCHANGED (team-menu keys already point at these sprites).
- Worked from Raven's LIVE system.sff (uploaded). fight.def from repo (it reflected the reported bug state: broken shimmer + silver fonts). If live fight.def differs, send it.

## P116 - resolution independence: NOT a rewrite. Only the power bars were off-canvas.
- Raven feared a big rewrite to support varying resolutions; at 1920x1080 the bottom bars vanished off-screen.
- DIAGNOSIS (no guesswork): system.def AND fight.def localcoord = 1280,720 (correct 16:9 design); GameWidth/Height=1280x720. The engine ALREADY scales localcoord-space UI to any resolution. Scanned every uncommented .pos/.offset: ONLY 2 elements sat past y=720 -> [Powerbar] p1.pos=574,901 + p2.pos=706,901. Comment even said ";~901 sits at screen bottom on 4:3". y=901 > 720 localcoord -> it lives in the EXTRA vertical band that a 4:3/Stage aspect renders BELOW the 720 canvas. On true 16:9 (1080p) the 720 canvas fills the screen exactly, so y>720 falls off the bottom. That's why it worked on 4:3/Stage but not 16:9.
- FIX: moved both power bars y 901 -> 675 (bars are top-anchored yaxis=0, bg ~28px*1.4=39, counter +20 -> span ~675-714, fully inside the 720 canvas). Now visible at ANY resolution/aspect; the engine scales uniformly. fight.def ONLY. Everything else was already within 720.
- TRADEOFF (told Raven): on his 4:3/Stage view the bars now sit ~70% down (canvas bottom) with empty space below them, instead of pinned to the 4:3 screen bottom. That's the cost of being aspect-independent (can't be at the absolute bottom of BOTH 4:3 and 16:9 with one static localcoord position). Exact bottom placement nudgeable with a 1080p screenshot.
- OPTIONAL (config.ini, NOT changed - Raven's preference): FightAspectWidth/Height currently -1,-1 (Stage aspect). For fully uniform behavior across users' monitors regardless of stage, set to 0,0 (resolution aspect). UI containment fix works regardless of this setting.
- KEY PRINCIPLE for future me: Ikemen UI is resolution-independent IF every element lives within the localcoord box. localcoord here is already 1280x720 - just never position UI past y=720 (or x past 1280). "Tuned for 4:3 bottom" = positioned in the off-canvas band = breaks on 16:9.

## P117 - hide stock render-Resolution option (lock the look) [external/script/options.lua]
- Traced the menu: the "Resolution" item is NOT in the screenpack's system.def - it's engine-default-generated from motif.option_info.menu.itemname_order, so there's no motif line to comment. options.lua already prunes menu items via main.f_pruneMenu(options.menu, excluded) at the end of menu build.
- FIX: added resolution=true + customres=true to the `excluded` table (both desktop + android branches). Cleanly HIDES the render-resolution submenu (which edits Video.GameWidth/Height = render res = rescales UI + zooms camera). The ['resolution']/['customres'] handlers stay defined -> functionality NOT removed, just pruned from display (exactly what Raven asked).
- HOW USERS SCALE NOW: the existing FULLSCREEN toggle upscales the locked 1280x720 render to any monitor (identical look). 'Default' resets GameWidth/Height to 1280x720 if a user previously changed it (options.lua lines 202-203 already set those).
- Based on REPO external/script/options.lua (engine scripts change rarely). If Raven's live differs, resend.
- NOT done this turn (honest scope): (1) a dedicated native-res OUTPUT scaler option (sets Video.WindowWidth/Height while keeping render 1280x720 -> sharper than fullscreen's 720 mode-switch). Repurposing the now-hidden resolution handlers to set WindowWidth/Height + lock render is the clean way but it's 4-5 coordinated UNTESTED Lua edits - not shipping blind. (2) Decorative Capcom-style ART bezels - needs a fullscreen overlay mechanism (ExternalShader + bezel texture, or a render mod) + the art assets; separate scoped job.
- ENGINE FACTS for future me: options menu items come from motif.option_info.menu.itemname_order (engine-default if motif omits it); hide via the `excluded` table + main.f_pruneMenu (itemname keys, e.g. 'resolution','customres','fullscreen','vsync','msaa','shaders'). hook system: hook.add(list,name,fn)/hook.run(list,...); 'options.default','options.menu.loop','options.save' fire. external/mods/*.lua auto-load.
